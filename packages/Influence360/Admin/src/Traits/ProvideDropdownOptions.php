...
namespace Influence360\Admin\Traits;
...
trait ProvideDropdownOptions
{
    ...
    {
        $options = app(\Influence360\User\Repositories\UserRepository::class)
    ...
    {
        $options = app(\Influence360\Contact\Repositories\OrganizationRepository::class)
            ->get(['id as value', 'name as label'])
            ->map(function ($item, $key) {
                $item->disabled = false;
